...
export async function generateRoutineAction(
  ...
  // Construcción dinámica de la estructura de días si es custom split
  let splitInstructions = "";
  if (userContext.generationType === 'day') {
    splitInstructions = `
  - MODO SESIÓN ÚNICA: Genera SOLO 1 DÍA de entrenamiento.
  - El usuario quiere una rutina para HOY basada en su prompt.
  - Volumen: 6-8 ejercicios.`;
  } else if (userContext.customSplit && userContext.customSplit.length > 0) {
    splitInstructions = `
  - ESTRUCTURA PERSONALIZADA (OBLIGATORIO):
  ${userContext.customSplit.map(d => `  Día ${d.day}: ${d.muscles.join(', ')}`).join('\n')}
  - Respeta estrictamente los grupos musculares asignados a cada día.`;
  } else {
    // Lógica estándar de presets
    splitInstructions = `
  - VOLUMEN DE LA SESIÓN: Genera OBLIGATORIAMENTE entre 6, 7 u 8 ejercicios por día (ni más, ni menos).${userContext.splitType === 'push-pull-legs' ? `\n  - PPL: Día 1=Push (pecho/hombro/tríceps), Día 2=Pull (espalda/bíceps), Día 3=Legs (cuádriceps/glúteos/isquios)` : ''}${userContext.splitType === 'upper-lower' ? `\n  - UPPER/LOWER: Alterna tren superior completo con tren inferior completo` : ''}${userContext.splitType === 'full-body' ? `\n  - FULL BODY: Cada día entrena todos los grupos musculares principales` : ''}${userContext.splitType === 'bro-split' ? `\n  - BRO SPLIT: Un grupo muscular principal por día (ej: Pecho, Espalda, Piernas, Hombros, Brazos)` : ''}`;
  }
  ...
  OPCIONES AVANZADAS (ALTA PRIORIDAD):${userContext.splitType ? `\n  - TIPO DE DIVISIÓN: ${userContext.splitType.toUpperCase()}` : ''}${userContext.sessionDuration ? `\n  - DURACIÓN POR SESIÓN: ${userContext.sessionDuration} minutos (ajusta volumen y ejercicios)` : ''}${userContext.intensity ? `\n  - INTENSIDAD: ${userContext.intensity.toUpperCase()} - RPE ${userContext.intensity === 'light' ? '5-6' : userContext.intensity === 'moderate' ? '7-8' : userContext.intensity === 'high' ? '8-9' : '9-10'}` : ''}${userContext.repRange ? `\n  - RANGO DE REPETICIONES: ${userContext.repRange} (USA ESTE RANGO en todos los ejercicios)` : ''}${userContext.specificFocus ? `\n  - ÉNFASIS: ${userContext.specificFocus.toUpperCase()} (diseña esquemas de series/reps optimizados para esto)` : ''}

  HISTORIAL DE ENTRENAMIENTO RECIENTE (CRÍTICO PARA RECUPERACIÓN):
  ${workoutHistorySummary || "No hay historial disponible."}
  
  INSTRUCCIONES DE RECUPERACIÓN Y VARIEDAD:
  1. ANALIZA LAS FECHAS Y EJERCICIOS: Si el usuario entrenó un grupo muscular ayer o anteayer (según las fechas del historial), EVITA programar ese músculo hoy como principal para permitir recuperación, A MENOS QUE el Split seleccionado lo requiera (ej: Full Body) o el usuario lo pida explícitamente.
  2. EVITA REPETICIÓN EXACTA: Si el último registro (ayer o antes de ayer) incluye exactamente los mismos ejercicios, intenta usar variantes (ej: cambiar Barra por Mancuernas) para ofrecer un estímulo novedoso, salvo que el objetivo sea Fuerza (donde la consistencia es clave).
  3. EQUILIBRIO: Si ves que ha entrenado solo "Empuje" recientemente, prioriza "Tracción" o "Pierna" si es una sesión única.
        ...
        "exercises": [
           {
              ...
              "grip": "Tipo de agarre y su efecto (Ej: 'Supino - Enfatiza cabeza corta')",
